...
class ReusedPySparkTestCase(unittest.TestCase):
    ...
    def setUpClass(cls, staticConfs=None):
        if staticConfs:
            # Passing a dict as staticConfs necessary for the test
            conf = SparkConf(loadDefaults=False)
            for k, v in staticConfs.items():
                conf.set(k, v)
            cls.sc = SparkContext('local[4]', cls.__name__, conf=conf)
        else:
...
class ContextTests(unittest.TestCase):
    ...
    def test_get_configentry(self):
        ...
        with SparkContext() as sc:
            ...
            max_task_failures_conf = sc._jvm \
                .__getattr__('org.apache.spark.internal.config.package$') \
                .__getattr__('MODULE$') \
                .MAX_TASK_FAILURES()
