...
class ReusedSQLTestCase(ReusedPySparkTestCase, SQLTestUtils):
    @classmethod
    def setUpClass(cls, staticConfs=None):
        ReusedPySparkTestCase.setUpClass(staticConfs)
...
class SQLTests(ReusedSQLTestCase):
    ...
    def test_stream_table_options(self):
        ...
        try:
            ...
            chk = os.path.join(tmpPath, '_chk')
            q = self.spark.readStream.table('tbl').writeStream.format('memory') \
                .queryName(tempTblName).start()
    ...
    def test_write_stream_table(self):
        ...
        try:
            ...
            chk = os.path.join(tmpPath, 'chk')
            q = self.spark.readStream.table(tblName1).writeStream.format('tahoe') \
                .option('checkpointLocation', chk).table(tblName2)
...
class HiveContextSQLTests(ReusedPySparkTestCase):
    ...
    def test_python_udf_on_top_window_function(self):
        from pyspark.sql import functions as F
        from pyspark.sql.window import Window
        from pyspark.sql.types import LongType
        df1 = self.spark.range(1, 4)
        square = F.udf(lambda x: x * x, LongType())
        w = Window.orderBy(F.col("id"))
        df2 = df1.select(
            F.col("id"),
            square(F.lag(F.col("id"), 1, -10).over(w)).alias("square_id_lag"))
        rs = sorted(df2.collect())
        expected = [
            (1, 100),
            (2, 1),
            (3, 4)
...
@unittest.skipIf(
    not _have_pandas,
    _pandas_requirement_message)
class TrustedFilePathTests(ReusedSQLTestCase):

    @classmethod
    def setUpClass(cls):
        staticConf = {
            "spark.databricks.pyspark.trustedJVMDataTransfer.enabled": "true",
            "spark.databricks.pyspark.maxTrustedTempDir": 1,
            "spark.databricks.pyspark.trustedTempDirSecondsToLive": 2
        }
        ReusedSQLTestCase.setUpClass(staticConf)
    ...
    def create_df_from_path_with_exception(self, test_path):
        ...
        msg = "java.lang.SecurityException: The path %s is not in a trusted path for " \
    ...
    def test_pandas_df_to_spark_df_with_trusted_path(self):
        import pandas
        pdf = pandas.DataFrame.from_items([('A', [1]), ('B', [2])])
        df = self.spark.createDataFrame(pdf)
        row = df.select('*').first()
        self.assertEqual(1, row[0])
        self.assertEqual(2, row[1])
    ...
    def test_df_from_list_with_trusted_path(self):
        l = [(1, 2)]
        df = self.spark.createDataFrame(l)
        row = df.select('*').first()
        self.assertEqual(1, row[0])
        self.assertEqual(2, row[1])
...
class FileBasedCollectTests(ReusedPySparkTestCase):
    ...
    def test_collect_take_and_first(self):
        self.assertEqual(self.spark.sql("select 1 as a").collect(), [Row(a=1)])
        self.assertEqual(self.spark.sql("select 1 as a").take(1), [Row(a=1)])
        self.assertEqual(self.spark.sql("select 1 as a").first(), Row(a=1))

    def test_arrow_is_unsupported(self):
        self.assertRaisesRegexp(
            Exception,
            "file-based collect",
            lambda: self.spark.range(10)._collectAsArrow())

    def test_toLocalIterator_is_unsupported(self):
        self.assertRaisesRegexp(
            Exception,
            "file-based collect",
            lambda: self.spark.range(10).toLocalIterator())
