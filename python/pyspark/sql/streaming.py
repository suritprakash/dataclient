...
class DataStreamWriter(object):
    ...
    @since(2.4)
    def table(self, tableName):
        """A method that mimics saveAsTable behavior for streaming. This method is only supported
        for Delta at the moment and should be pushed to open source.

        The behavior is as follows:
         - If the table already exists, and is a Delta table, continue writing to this table
         - If the table already exists, and is not a Delta table, throw error
         - If the table doesn't exist and format is Delta, and path is specified:
           create external Delta table, and start streaming into it.
         - If the table doesn't exist and format is Delta, and path is not specified: create managed
           Delta table, and start streaming into it

        :param tableName: The name of the table
        """
        if isinstance(tableName, basestring):
            return self._sq(self._jwrite.table(tableName))
        else:
            raise TypeError("tableName can be only a single string")
