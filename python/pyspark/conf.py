...
class SparkConf(object):
    ...
    def get(self, key, defaultValue=None):
        ...
        # Hack to support reading typed configs in Python:
        if isinstance(key, JavaObject) \
                and key.getClass().getName().startswith("org.apache.spark.internal.config"):
            if defaultValue is not None:
                raise Exception(
                    "Do not pass defaultValue when calling get() with a JVM ConfigEntry object")
